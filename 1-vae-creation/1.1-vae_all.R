...
# create_samples(all_images_path, VAES, train_num, val_num, test_num, dataset_seed, incl_wash = FALSE, thresh, pattern = "bmp$|BMP$")
...
gen_x_decoded_mean_squash_crp <- decoder_mean_squash_crp(gen_x_decoded_mean_squash)
generator <- keras_model(gen_decoder_input, gen_x_decoded_mean_squash_crp)
...
if (visualization) {
  ...
  # Decoder function:
  decode_image <- function(latent_vector) {
    hidden_decoded <- decoder_hidden(latent_vector)
    up_decoded <- decoder_upsample(hidden_decoded)
    reshape_decoded <- decoder_reshape(up_decoded)
    deconv_1_decoded <- decoder_deconv_1(reshape_decoded)
    deconv_2_decoded <- decoder_deconv_2(deconv_1_decoded)
    x_decoded_relu <- decoder_deconv_3_upsample(deconv_2_decoded)
    x_decoded_mean_squash <- decoder_mean_squash(x_decoded_relu)
    x_decoded_mean_squash_crp <- decoder_mean_squash_crp(x_decoded_mean_squash)
    
    return(x_decoded_mean_squash_crp)
  }
  
  ## DOESN'T WORK... All reconstructed images have the same pixels..
  
  image_index <- 300
  
  x_test_matrix <- x_test_encoded %>% as.matrix()
  x_test_array <- array(x_test_matrix, dim = c(nrow(x_test_encoded), ncol(x_test_encoded)))
  x_test_decoded <- predict(generator, x_test_array,batch_size = batch_size)
  decoded_df <- x_test_decoded %>% as_data_frame()
  
  dec1 <- decode_image(x_test_array[1,])
  dec1 <- decode_image(x_test_array[2,])
  
  image_matrix <- x_test_decoded[image_index,,,] ## GRABS FIRST IMAGE
  # Transpose the matrix to match the image() function's requirements
  image_matrix <- t(image_matrix)
  # Reverse the rows (required due to the difference in the coordinate system)
  image_matrix <- image_matrix[nrow(image_matrix):1,]
  # Plot the image
  image(image_matrix, col = gray.colors(256))
  
  original_image <- load.image(files_x_test[image_index])
  plot(grayscale(original_image))
  
  
  ### IMAGE INTERPOLATION ###
  ## ALSO DOESN'T WORK BECAUSE DECODER/GENERATOR DOESNT WORK
  imgs_encoded <- predict(encoder, x_test, batch_size = batch_size)
  latent_vector1 <- imgs_encoded[200,]
  latent_vector2 <- imgs_encoded[300,]
  # Define the interpolation parameter (alpha)
  alpha <- seq(0, 1, length.out = 5)
  # Perform the interpolation in the latent space and decode the interpolated vectors
  interpolated_vectors <- sapply(alpha, function(a) a * latent_vector1 + (1 - a) * latent_vector2)
  interpolated_images <- apply(interpolated_vectors, 2, generator)
  # Display the interpolated images
  interpolated_plots <- lapply(1:ncol(interpolated_images), function(i) display_image(interpolated_images[, , i]))
  gridExtra::grid.arrange(grobs = interpolated_plots, ncol = length(alpha))
