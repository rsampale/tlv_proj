...
create_save_subimages <- function(image_crop,name) {
    ...
    for (i in 1: 8) {
        for (j in 1: 12) {
            ...
            # dev.off()
            imager::save.image(im=tempimg,file=paste("/home/data/refined/candescence/tlv/0.2-images_cut/all-batch2/",name,"-r",i,"-c",j,".bmp",sep = ""))
        }
    }
}

image_files <- list.files(path = "/home/data/refined/candescence/tlv/0.1-images_whole/day-5-batch2",pattern="*.bmp",full.names = TRUE,recursive = FALSE)
